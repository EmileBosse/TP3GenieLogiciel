<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"  xmlns:uFN="uFN" version="2.0">
  ...
  <xsl:template match="Project/Models">
	   ...
	   <xsl:result-document href="{$projectName}/DAL/MSSQL/Database.cs">
using System;
    ...
    public static class DataBase
    {
        ...
        public static DataTable SelectTable(string psTableName, string psOrderBy)
        {
            try
            {
                ...
                if (psOrderBy != " " <xsl:text>&#038;&#038;</xsl:text> !psOrderBy.ToUpper().Contains("ORDER BY"))
                {
        ...
        public static DataRow SelectRow(string psTableName, string psWhere)
        {
            ...
            if (psWhere != " " <xsl:text>&#038;&#038;</xsl:text> !psWhere.ToUpper().Contains("WHERE"))
            {
        ...
        public static int Insert(string psTableName, string[] psValues)
        {
            ...
            if (!Util.isNULL(psValues))
            {
                ...
                if (liNbValues != 0 <xsl:text>&#038;&#038;</xsl:text> (liValuesLength % 2) == 0)
                {
                    for (int i = 0; i <xsl:text>&lt;</xsl:text> liNbValues; i++)
                    {
                        lsCols += psValues[2 * i];
        ...
        public static int Insert(string psTableName, string[] psCols, string[] psValues)
        {
            ...
            if (!(Util.isNULL(psCols) || Util.isNULL(psValues)))
            {
                ...
                if (liColsLength != 0 <xsl:text>&#038;&#038;</xsl:text> liColsLength == psValues.Length)
                {
                    string lsQuery = "";
                    string lsCols = "";
                    string lsValues = "";

                    for (int i = 0; i <xsl:text>&lt;</xsl:text> liColsLength; i++)
                    {
                        lsCols += psCols[i];
                        lsValues += psValues[i];
                        if (i != (liColsLength - 1))
